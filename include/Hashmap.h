...
public:
    ...
    /**
     * @brief adds the key value pair inside a hashmap
     ...
     * @param key is the key whose value is to be updated
     ...
     * @return value if found else default value
     */
    Value get(Key key) {
        int64_t hashed = hash(key) % capacity;
        Value result;
    ...
    /**
     * @brief fetches the value pointer from the node inside the hashmap
     * @param key is the key to fetch pointer for
     * @return pointer to value if found else nullptr
     */
    ...
    /**
     * @brief gets the size of hashmap
